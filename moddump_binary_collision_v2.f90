...
module moddump
   ...
   subroutine read_setupfile(filename,ierr)
    ...
    call read_inopt(n_id,    'n_id',    db,errcount=nerr)
    call read_inopt(mh,    'mh',    db,errcount=nerr)
    call read_inopt(rs,    'rs',    db,errcount=nerr)
 
    call read_inopt(ecc1,   'ecc1',   db,errcount=nerr)
    call read_inopt(mp1,    'mp1',    db,errcount=nerr)
    call read_inopt(rp1,    'rp1',    db,errcount=nerr)
    call read_inopt(a1,    'a1',    db,errcount=nerr)
    call read_inopt(x1, 'x1', db,errcount=nerr)
    call read_inopt(y1, 'y1', db,errcount=nerr)
    call read_inopt(v_x1, 'v_x1', db,errcount=nerr)
    call read_inopt(v_y1, 'v_y1', db,errcount=nerr)

 
    call read_inopt(ecc2,   'ecc2',   db,errcount=nerr)
    call read_inopt(mp2,    'mp2',    db,errcount=nerr)
    call read_inopt(rp2,    'rp2',    db,errcount=nerr)
    call read_inopt(a2,    'a2',    db,errcount=nerr)
    call read_inopt(x2, 'x2', db,errcount=nerr)
    call read_inopt(y2, 'y2', db,errcount=nerr)
    call read_inopt(v_x2, 'v_x2', db,errcount=nerr)
    call read_inopt(v_y2, 'v_y2', db,errcount=nerr)  
